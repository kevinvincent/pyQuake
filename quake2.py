...
PyQuake supports two "profiles" that can be used to analyze multiple
data files quickly without modifying settings everytime.
...
class mainGUI:
    ...
    def askForFile(self):
        """
        Open select file dialog, get filepath, and also set the selected file label on GUI
        """
        global filepath;
...
class reportDaemon:
    ...
    def saveReport(self):
        """
        Opens save file dialog and writes to selected filename
        """
        try:
